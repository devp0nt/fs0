...
export class Fs0 {
  ...
  toAbs<T extends Fs0.PathOrPaths>(
    path: T,
    relativeToAbs: string = this.cwd
  ): T {
  ...
  toRel<T extends Fs0.PathOrPaths>(
    path: T,
    relativeTo?: string,
  ...
  writeFileSync(path: string, content: string, format: boolean = false) {
    const absPath = this.toAbs(path);
    fsSync.mkdirSync(nodePath.dirname(absPath), { recursive: true });
    fsSync.writeFileSync(absPath, content);
    if (format) {
      this.formatFileSync(absPath);
    }
  }
  ...
  static sortJson = <T>(
    content: T,
    sort: true | string[] | ((content: T) => string[]) = true
  ): T => {
    ...
    for (const key of keys) {
      ...
      if (Array.isArray(sort)) {
        ...
        const dottedKeys = sort
          .filter((k) => k.startsWith(key + "."))
          .map((k) => k.slice(key.length + 1)); // cut off prefix "key."
  ...
  writeJsonSync<T>(
    ...
    this.writeFileSync(
      path,
      CommentJson.stringify(sortedContent, null, 2),
      format
    );
  }
  async writeJson<T>(
  ...
  toPaths(path: Fs0.PathOrPaths): string[] {
    return Array.isArray(path)
      ? path.map(this.normalizePath.bind(this))
      : [this.normalizePath(path)];
  }

  isPathMatchGlob(path: string, glob: string | string[]): boolean {
    const pathNormalized = this.normalizePath(path);
    const globNormalized = Array.isArray(glob)
      ? glob.map(this.normalizePath.bind(this))
      : [this.normalizePath(glob)];
    const negativeGlobs = globNormalized
  ...
  isPathInDir(path: string, dir: string): boolean {
    ...
    return (
      pathNormalized.startsWith(dirNormalized) &&
      pathNormalized !== dirNormalized
    );
  }
  ...
  static async findUp(
    ...
    createFsInput?: Fs0.CreateFsInput
  ) {
  ...
  async loadEnv(filename: string = ".env"): Promise<Record<string, string>> {
    return dotenv.config({ path: await this.findUp(filename) })
      .parsed as Record<string, string>;
  }

  loadEnvSync(filename: string = ".env"): Record<string, string> {
    return dotenv.config({ path: this.findUpSync(filename) }).parsed as Record<
      string,
      string
    >;
  }
  ...
  async importFreshDefault<T = any>(path: string): Promise<T> {
    path = this.toAbs(path);
    return (await import(`${path}?t=${Date.now()}`).then(
      (m) => m.default
    )) as T;
  }

  async importFresh1<T = any>(
    path: string,
  ...
  async importFreshDefault1<T = any>(
    path: string,
    alias?: Record<string, string>
  ): Promise<T> {
    path = this.toAbs(path);
...
export class File0 {
  ...
  async importFreshDefault<T = any>(): Promise<T> {
    return (await import(`${this.path.abs}?t=${Date.now()}`).then(
      (m) => m.default
    )) as T;
  }
...
export class Formatter0 {
  ...
  static async create(props?: {
    ...
    if (props?.command) {
      ...
      const {
        ...
        eslintConfigFile0,
        prettierConfigFile0,
      } = await Formatter0.detectTools({ fs0 });
      return new Formatter0({
        tools,
        command: props?.command,
        fs0,
  ...
  static async detectTools({ fs0 }: { fs0: Fs0 }): Promise<{
    ...
    const biomeConfigFile0 = await fs0.findUpFile([
      "biome.json",
      "biome.jsonc",
    ]);
    if (biomeConfigFile0) {
